...
const App = () => {
  ...
  return (
    <div className="min-h-screen bg-neon-dark text-neon-green font-mono selection:bg-neon-green selection:text-black relative">
      ...
      {/* Hero Section */
      /*<div 
        className={`fixed inset-0 transition-transform duration-1000 ease-in-out z-50 bg-neon-dark ${
      ...
      >
        ...
        /*<nav className="fixed top-0 left-0 right-0 z-40 bg-neon-dark/90 backdrop-blur-md border-b border-neon-green/20">
          <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
            <div className="text-neon-green font-bold text-xl tracking-tighter hover:text-neon-cyan transition-colors cursor-pointer">
              ~/SOHAM
            </div>
            ...
            {/* Mobile Toggle */
             /*<button 
              className="md:hidden text-neon-green border border-neon-green/30 px-3 py-1 text-sm hover:bg-neon-green/10"
          ...
          /*{mobileMenuOpen && (
            <div className="md:hidden absolute top-full left-0 w-full bg-black border-b border-neon-green/20 p-4 flex flex-col gap-4 shadow-[0_4px_20px_rgba(0,255,65,0.1)]">
              <a href="#about" onClick={closeMobileMenu} className="block py-2 text-neon-green hover:pl-2 transition-all border-l-2 border-transparent hover:border-neon-green pl-1"> &gt; ABOUT_ME</a>
        ...
        /*<div className="pt-24 px-4 pb-20 max-w-6xl mx-auto">
          ...
          <footer className="text-center text-neon-green/40 text-xs py-8 border-t border-neon-green/10 mt-12">
            ...
            <p>&copy; 2025 SOHAM SHETYE. ALL RIGHTS RESERVED.</p>
          </footer>
        </div>
      </main>
    </div>
...
import BootLoader from './components/BootLoader'; // IMPORT NEW LOADER
import Hero from './components/Hero';
import About from './components/About';
import Skills from './components/Skills';
import Projects from './components/Projects';
...
const App = () => {
  ...
  // OPTIONAL: Wait for window load + boot sequence
  useEffect(() => {
    const handleLoad = () => {
      // You can remove this listener logic if you just want the timer
      // but keeping it ensures images are ready.
    };
  ...
  return (
    <>
      {isLoading ? (
        ...
        <div className="min-h-screen bg-neon-dark text-neon-green font-mono selection:bg-neon-green selection:text-black relative">
           {/* ... REST OF YOUR APP CODE (Hero, Main, etc) ... */}
           {/* (Paste the rest of the layout here exactly as it was) */}
           ...
           <main className={`transition-opacity duration-1000 ease-in-out ${showContent ? 'opacity-100' : 'opacity-0'}`}>
             <nav className="fixed top-0 left-0 right-0 z-40 bg-neon-dark/90 backdrop-blur-md border-b border-neon-green/20">
               <div className="max-w-5xl mx-auto px-4 py-3 flex justify-between items-center">
                 ...
                 <button className="md:hidden text-neon-green border border-neon-green/30 px-3 py-1" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                    {mobileMenuOpen ? '[ X ]' : '[ MENU ]'}
                 </button>
               </div>
               {mobileMenuOpen && (
                 <div className="md:hidden absolute top-full left-0 w-full bg-black border-b border-neon-green/20 p-4 flex flex-col gap-4">
